...
nome_modelo = 'ResNet-FP32'
diretorio_pai = criar_diretorio_incrementado('resultadosResNet', nome_modelo)
...
class ResNet34(nn.Module):
    def __init__(self, num_classes=10):
        ...
        # Conv inicial reduzido: 3x3, sem maxpool
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)  # 3 blocos
        x = self.layer2(x)  # 4 blocos
        x = self.layer3(x)  # 6 blocos
        x = self.layer4(x)  # 3 blocos
...
# @track_emissions(project_name="ResNet34_original", output_dir=diretorio_carbon)
