...
nome_modelo = 'GoogleNet-FP32'
diretorio_pai = criar_diretorio_incrementado('resultadosGoogleNet', nome_modelo)
print(f'Diretório criado: {diretorio_pai}')

# Cria o diretório 'AlexNetCarbon'
diretorio_carbon = criar_diretorio_incrementado(diretorio_pai, 'GoogleNet_carbono')
...
class GoogleNet(nn.Module):
    def __init__(self, num_classes=10, dropout_rate=0, width_multiplier=1):
        super(GoogleNet, self).__init__()
        
        # Estágio inicial
        self.pre_layers = nn.Sequential(
            nn.Conv2d(3, int(64 * width_multiplier), kernel_size=3, stride=1, padding=1),
        ...
        self.inception_3a = InceptionModule(
            int(64 * width_multiplier), 64, 96, 128, 16, 32, 32, width_multiplier
        )
        self.inception_3b = InceptionModule(
            int(256 * width_multiplier), 128, 128, 192, 32, 96, 64, width_multiplier
        )
    ...
    def forward(self, x):
        ...
        # Pooling adaptativo e classificação
        x = self.avg_pool(x)
        x = x.view(x.size(0), -1)
        x = self.dropout(x)
        x = self.linear(x)
...
modelo = GoogleNet().to(dispositivo)
...
# @track_emissions(project_name="GoogleNet_original", output_dir=diretorio_carbon)
